<img src="<?php echo URL::webapp(); ?>img/pesquisar_encontrados.svg" id="pesquisapet" class="pesquisapet"  alt="Adicionar Pet">

<header class="cabecalho">
  <h1>Encontrados</h1>
  <h2>Veja os pets que foram encontrados e aparentemente pertenciam a um lar.</h2>
</header>


<!-- ===========PESQUISA PET=========== -->
<form id="formulario" class="formulario" action="encontrados" method="post" enctype="multipart/form-data">
  ...
  <div class="form-section">
    ...
    <?php $estado_id = SS::sss('_ss_sppe_', 'estado') ?? "0"; ?>
    <select id="estado" name="estado" value="<?php echo $estado_id; ?>"></select><br><br>

    <label for="cidade">Cidade</label><br>
    <?php $cidade_id = SS::sss('_ss_sppe_', 'cidade') ?? "0"; ?>
    ...
    <select id="especie" name="especie">
      <?php
        ...
        foreach ($options as $key => $value)
          if ($value == SS::sss('_ss_sppe_', 'especie'))
  ...
  <div class="form-section">
    ...
    <select id="genero" name="genero">
      <?php
        ...
        foreach ($options as $key => $value)
          if ($value == SS::sss('_ss_sppe_', 'genero'))
            echo '<option selected value="'.$value.'">'.$key.'</option>';
          else
            echo '<option value="'.$value.'">'.$key.'</option>';
      ?>
    ...
    <select id="idade" name="idade">
      <?php
        ...
        foreach ($options as $key => $value)
          if ($value == SS::sss('_ss_sppe_', 'idade'))
            echo '<option selected value="'.$value.'">'.$key.'</option>';
          else
            echo '<option value="'.$value.'">'.$key.'</option>';
      ?>
    ...
    <select id="porte" name="porte">
      <?php
        ...
        foreach ($options as $key => $value)
          if ($value == SS::sss('_ss_sppe_', 'porte'))
            echo '<option selected value="'.$value.'">'.$key.'</option>';
          else
            echo '<option value="'.$value.'">'.$key.'</option>';
      ?>
    ...
    <select id="castrado" name="castrado">
      <?php
        ...
        foreach ($options as $key => $value)
          if ($value == SS::sss('_ss_sppe_', 'castrado'))
            echo '<option selected value="'.$value.'">'.$key.'</option>';
          else
            echo '<option value="'.$value.'">'.$key.'</option>';
      ?>
  ...
  <div class="form-section">
    ...
    <select id="vacinado" name="vacinado">
      <?php
        $options = ['Todos'=>'','Não'=>'Não','Sim'=>'Sim'];
        foreach ($options as $key => $value)
          if ($value == SS::sss('_ss_sppe_', 'vacinado'))
            echo '<option selected value="'.$value.'">'.$key.'</option>';
          else
            echo '<option value="'.$value.'">'.$key.'</option>';
      ?>
    ...
    <input type="text" name="nome" id="nome" maxlength="20" value="<?php echo SS::sss('_ss_sppe_', 'nome'); ?>" placeholder="..." ><br><br>

    <label for="descricao">Descrição</label><br>
    <input type="text" name="descricao" id="descricao" maxlength="600"  value="<?php echo SS::sss('_ss_sppe_', 'descricao'); ?>" placeholder="..."><br><br>
...
<div class="show-pet" id="show_pet_conteiner">
  ...
  <div class="show-pet-option">
    <button type="button" name="button" class="add-lista-adocao"  onclick="add_lista_adocao()" title="Criar um vínculo para que a entidade que encontrou o pet saiba que você é o guardião.">Criar Vínculo</button>
    ...
    <?php if (URL::is_mobile()): ?>
      <div class="show-pet-option">
        <button type="button" name="button" class="add-lista-adocao"  onclick="add_lista_adocao()" title="Sou o guardião desse pet!">Sou o guardião desse pet</button>
        <button type="button" name="button" class="show-pet-denuncie" onclick="denunciar()" title="Isso não é um pet!">Denúncie</button>
        <button type="button" name="button" class="show-pet-close" onclick="invisible_pet()">Fechar</button>
      </div>
    <?php endif; ?>
...
<?php else: ?>
  <?php foreach ($pets as $value): ?>
    <div class="pet-box encontrado" onclick="show_pet(<?php echo $value['id']; ?>, this)">
      <div class="img-box">
...
if ($urlpage > 0 || $num_pets>=$limit): ?>
  <div class="paginacao">
    <?php if ($urlpage > 0): $voltar = $urlpage - 1; ?>
      <a href="<?php echo "encontrados/".$voltar; ?>"  class="botao-pag voltar">Voltar</a>
    <?php endif; ?>
    <?php if ($num_pets>=$limit): $mais_pets = $urlpage + 1; ?>
      <a href="<?php echo "encontrados/".$mais_pets; ?>" class="botao-pag mais-pets">Mais Pets</a>
    <?php endif; ?>
  </div>
<?php endif; ?>

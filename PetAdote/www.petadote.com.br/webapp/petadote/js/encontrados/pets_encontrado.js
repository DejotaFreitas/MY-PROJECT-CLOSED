function add_lista_adocao() {
  var titulo = "Vincular pet encontrado ?";
  var msg = "Se esse pet aparentemente é seu, e foi encontrado por outra pessoa, vincule-se a ele, para demostrar que o pet estava sobre sua guarda antes de se perder, lembrando também que esse pet ficará salvo no menu vínculos.";
...
function peencher_show_pet(pet) {
  ...
  pet_situacao.innerHTML = "Encontrado";
...
function ajax_add_lista_adocao(id_pet){
	try {
		...
		ajax.open("POST", "ajax_dono_encontrado", true);
		ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		ajax.send("json="+JSON.stringify({'id_pet':id_pet}));
	} catch (e) { console.log("ERRO: "+ e.message);	}
}

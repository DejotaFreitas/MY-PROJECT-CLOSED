function add_lista_adocao() {
  var titulo = "Vincular pet para adoção ?";
  var msg = "Se deseja adotar esse pet, vincule-se a ele, para que seja adicionado na lista de pessoas interessdas desse pet, lembrando também que esse pet ficará salvo no menu vínculos.";
...
function ajax_add_lista_adocao(id_pet){
	try {
		...
		ajax.open("POST", "ajax_add_lista_adocao", true);
		ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		ajax.send("json="+JSON.stringify({'id_pet':id_pet}));
	} catch (e) { console.log("ERRO: "+ e.message);	}
}
